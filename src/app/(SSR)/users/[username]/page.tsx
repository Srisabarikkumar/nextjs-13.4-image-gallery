...
// const getUserCached = cache (getUser); Use cache if you're not using the native fetch API

export async function generateMetadata({ params: { username } }: pageParams): Promise<Metadata> {
...
export default async function userProfilePage({ params: { username } }: pageParams) {
    const user = await getUser(username);
